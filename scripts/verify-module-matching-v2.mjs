...
// 专业代码映射
const disciplineCodeMapping = eventRegistry.discipline_code_mapping || {};
...
function checkEventMatching() {
  ...
  for (const [eventType, event] of Object.entries(eventTypes)) {
    ...
    for (const producer of producers) {
      ...
      // 使用专业代码映射
      const mapping = disciplineCodeMapping[producer];
      const producerSkill = skillIndex.skills.find(s => 
        s.group === producer || 
        s.discipline === producer ||
        (mapping && s.group === mapping.skill_group)
      );
      
      if (!producerSkill && !mapping) {
        addIssue('warning', '事件匹配', `事件 ${eventType} 的生产者 ${producer} 在Skill中不存在`);
      } else if (producerSkill) {
    ...
    for (const subscriber of subscribers) {
      ...
      // 使用专业代码映射
      const mapping = disciplineCodeMapping[subscriber];
      const subscriberSkill = skillIndex.skills.find(s => 
        s.group === subscriber || 
        s.discipline === subscriber ||
        (mapping && s.group === mapping.skill_group)
      );
      
      if (!subscriberSkill && !mapping) {
...
function checkFormulaMatching() {
  ...
  console.log("公式库专业分布:", formulaDisciplines);
  
  // 专业代码到公式库专业名称的映射
  const disciplineToFormula = {
    "PR": "process",
    "WA": "water",
    "PI": "piping",
    "EL": "electrical",
  ...
  for (const [discipline, count] of Object.entries(skillDisciplines)) {
    ...
    const formulaDiscipline = disciplineToFormula[discipline] || discipline.toLowerCase();
    const formulaCount = formulaDisciplines[formulaDiscipline] || formulaDisciplines[discipline] || 0;
...
function checkKnowledgeMatching() {
  ...
  for (const [discipline, count] of Object.entries(skillDisciplines)) {
    ...
    const knowledgeDiscipline = disciplineToKnowledge[discipline] || discipline.toLowerCase();
    const clauseCount = knowledgeDisciplines[knowledgeDiscipline] || knowledgeDisciplines[discipline] || 0;
...
function checkCrossDisciplineEvents() {
  ...
  for (const eventType of criticalEvents) {
    ...
    if (event) {
      ...
      // 检查是否有生产者
      if (!event.produced_by || event.produced_by.length === 0) {
        addIssue('error', '跨专业提资', `事件 ${eventType} 缺少生产者`);
      }
      
      // 检查是否有订阅者（支持两种格式）
      const hasSubscribers = (event.subscribers && event.subscribers.length > 0) ||
                             (event.subscribers_by_discipline && Object.keys(event.subscribers_by_discipline).length > 0);
...
function generateReport() {
  ...
  // 保存报告
  const reportPath = path.join(ROOT, "pilotdeck-vdi/tests/module-matching-report-v2.json");
