...
/**
 * 工艺知识库检索回归测试
 * 用法: node pilotdeck-vdi/scripts/test-process-knowledge-retrieval.mjs
 */
...
const MANIFEST = path.join(ROOT, "data/process-knowledge-manifest.json");

function mcpSearch(query, discipline = "process") {
  ...
  const tmp = path.join(os.tmpdir(), `vdi-kb-test-${Date.now()}.json`);
  try {
