...
// 按专业过滤
const search6 = searchFormulas("水头损失", "water");
assert(search6.every(f => f.discipline === "water"), "专业过滤生效");

// --- 测试 3: 公式详情 ---
section("3. 公式详情 vdi_get_formula");

const f1 = loadFormulaDetail("WA-HYD-001");
assert(f1 !== null, "WA-HYD-001 加载成功");
assert(f1.formula_id === "WA-HYD-001", "公式ID正确");
...
const f2 = loadFormulaDetail("WA-RAI-002");
assert(f2 !== null, "WA-RAI-002 加载成功");
...
// 4.1 海曾-威廉姆斯水头损失
const calc1 = executeSingleFormula("WA-HYD-001", { L: 100, Q: 0.05, C: 130, D: 0.2 });
assertApprox(calc1.result_value, 1.281202, 0.001, "WA-HYD-001: Hf ≈ 1.281m (L=100,Q=0.05,C=130,D=0.2)");
assert(calc1.audit.evidence_tag.includes("GB 50013-2018"), "包含 evidence_tag");

// 4.2 单位转换: mm → m
const calc1b = executeSingleFormula("WA-HYD-001", { L: 100, Q: 0.05, C: 130, D: 200 }, { D: "mm" });
assert(calc1b.conversions.length > 0, "单位转换记录存在");
assertApprox(calc1b.result_value, 1.281202, 0.001, "WA-HYD-001: mm转m后结果一致");

// 4.3 曼宁公式
const calc2 = executeSingleFormula("WA-HYD-002", { n: 0.013, R: 0.05, S: 0.005 });
...
try {
  executeSingleFormula("WA-HYD-001", { L: -10, Q: 0.05, C: 130, D: 0.2 });
} catch (e) {
...
try {
  executeSingleFormula("WA-HYD-001", { L: 100, Q: 0.05, C: 130, D: 5 });
} catch (e) {
...
try {
  executeSingleFormula("WA-HYD-001", { L: 100 });
} catch (e) {
...
// 场景：给水管段最不利点水头计算
// 步骤1: 沿程水头损失 (WA-HYD-001)
// 步骤2: 局部水头损失 (WA-HYD-005, 输入=步骤1输出)
// 步骤3: 水泵轴功率 (WA-EQ-001, H=步骤1+步骤2)
const pipeline = [
  { step: 1, formula_id: "WA-HYD-001", inputs: { L: 200, Q: 0.03, C: 130, D: 0.15 } },
  { step: 2, formula_id: "WA-HYD-005", inputs: { Hf: { from_step: 1, symbol: "Hf" }, k: 0.25 } },
  { step: 3, formula_id: "WA-EQ-001", inputs: {
    Q: 0.03,
    H: null, // 将在下面计算
    η: 0.7,
  }},
...
// 手动执行组合计算验证
const step1 = executeSingleFormula("WA-HYD-001", { L: 200, Q: 0.03, C: 130, D: 0.15 });
const step2 = executeSingleFormula("WA-HYD-005", { Hf: step1.result_value, k: 0.25 });
const totalH = step1.result_value + step2.result_value;
const step3 = executeSingleFormula("WA-EQ-001", { Q: 0.03, H: totalH, η: 0.7 });
...
// 测试 from_step 引用解析
const pipelineSteps = [
  { step: 1, formula_id: "WA-HYD-004", inputs: { D: 0.2 } },
  { step: 2, formula_id: "WA-HYD-002", inputs: { n: 0.013, R: { from_step: 1, symbol: "R" }, S: 0.005 } },
];
...
for (const id of hydraulicNew) {
  assert(allIds.includes(id), `索引包含新水力学公式 ${id}`);
}

// 工艺专业
const processFormulas = allIds.filter(id => id.startsWith("PR-"));
assert(processFormulas.length >= 4, `工艺专业包含 ${processFormulas.length} 条公式 (≥4)`);

// 管道专业
const pipingFormulas = allIds.filter(id => id.startsWith("PI-"));
assert(pipingFormulas.length >= 3, `管道专业包含 ${pipingFormulas.length} 条公式 (≥3)`);

// 电气专业
const electricalFormulas = allIds.filter(id => id.startsWith("EL-"));
assert(electricalFormulas.length >= 3, `电气专业包含 ${electricalFormulas.length} 条公式 (≥3)`);

// HSE专业
const hseFormulas = allIds.filter(id => id.startsWith("HS-"));
assert(hseFormulas.length >= 2, `HSE专业包含 ${hseFormulas.length} 条公式 (≥2)`);

// --- 测试 9: 跨专业公式计算 ---
section("9. 跨专业公式计算");

// 管道壁厚
const pipingCalc = loadFormulaDetail("PI-STR-001");
if (pipingCalc) {
  const r = executeSingleFormula("PI-STR-001", { P: 1.6, D: 219, S: 137, E: 1.0, C: 1.5 });
  ...
  skipped++;
  console.log("  ⊘ PI-STR-001 未找到，跳过");
}

// 三相电机功率
const elecCalc = loadFormulaDetail("EL-POW-001");
if (elecCalc) {
  const r = executeSingleFormula("EL-POW-001", { U: 0.38, I: 50, cosphi: 0.85 });
  assert(r.result_value > 0, `三相功率 P = ${r.result_value.toFixed(2)} kW`);
} else {
  skipped++;
  console.log("  ⊘ EL-POW-001 未找到，跳过");
}

// 通风换气次数
const hseCalc = loadFormulaDetail("HS-VEN-001");
if (hseCalc) {
  const r = executeSingleFormula("HS-VEN-001", { n: 6, V: 500 });
  assertApprox(r.result_value, 3000, 0.1, "Q=6*500=3000 m³/h");
} else {
  skipped++;
  console.log("  ⊘ HS-VEN-001 未找到，跳过");
}
